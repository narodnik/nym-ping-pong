...
async def find_client(nym, key_id):
    clients = await nym.clients()
    return [client for client in clients if client.Id == key_id][0]
...
async def run_client():
    ...
    key_id = "F9xzbjnMQVN4ZidcqN2ip9kVnI9wbS39aVayZGiMihY="
    client = await find_client(nym, key_id)
    print(client)

    message = proto.message.ChatMessage(
        Content=b"hello howdy hihi",
        SenderPublicKey=my_details.PubKey,
        SenderProviderPublicKey=None,
        MessageNonce=110,
        SenderTimestamp=time.time_ns(),
        Signature=None
    )
    message_data = message.SerializeToString()

    await nym.send(message_data, client)
